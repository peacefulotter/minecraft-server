...
import { String, UUID } from '~/types/basic'
import {
    EncryptionRequest,
    HandshakeResponse,
    LoginSuccess,
...
export class ConnectionHandler {
    ...
    handleLoginStart = ({ client, buffer }: HandlerArgs) => {
        ...
        log(
            'Attempting to log',
            chalk.yellow(packet.username)
            // chalk.blue(packet.uuid)
        )

        const clientUuid = uuid()
        console.log(clientUuid, UUID.write(clientUuid))

        // client.state = ClientState.PLAY
        // console.log({
        //     // uuid: packet.uuid,v > 1.8.0
        //     uuid: Buffer.allocUnsafe(16),
